...
def run_momentum(df, available_funds = FUNDS):
	if VERBOSE:
...
def main():
	...
	for look in [i * UNITS for i in range(5,61,5)]:
		returns = []
		LOOKBACK = look

		pnl, indicators_buy, indicators_sell, buy_annotations, sell_annotations, pnls = run_momentum(df, available_funds=FUNDS)
		returns.append(pnl)
